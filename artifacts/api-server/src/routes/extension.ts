...
import { ExtensionAgentBody } from "@workspace/api-zod";

const router = Router();

type PageElement = { id: string; tag: string; text: string; ariaLabel?: string | null; placeholder?: string | null; href?: string | null };
type PageContext = { url: string; title: string; elements: PageElement[] };
type AgentAction = { type: string; elementId?: string | null; value?: string | null; direction?: string | null; url?: string | null };
type AgentPlan = { response: string; action?: AgentAction; blocked?: boolean | null };
...
function elementLabel(el: PageElement) {
  return normalize([el.text, el.ariaLabel, el.placeholder, el.href, el.tag].filter(Boolean).join(" "));
}

function isRisky(command: string) {
  const n = normalize(command);
  return RISKY_WORDS.some((w) => n.includes(w));
}
...
function findNavElement(command: string, context: PageContext) {
  ...
  for (const [target, aliases] of Object.entries(NAV_TARGETS)) {
    ...
    const match = context.elements.find((el) => {
      const label = elementLabel(el);
      return label.includes(target) || aliases.some((a) => label.includes(a));
    });
...
export function planAction(command: string, context: PageContext): AgentPlan {
  const nc = normalize(command);
  if (isRisky(command)) {
    return { blocked: true, response: "I can help you navigate there, but I will not submit, delete, or start graded work without a confirmation flow." };
  }
...
router.post("/extension/agent", async (req, res): Promise<void> => {
  const user = await requireAuth(req, res);
  if (!user) return;

  const parsed = ExtensionAgentBody.safeParse(req.body);
  if (!parsed.success) {
  ...
  const plan = planAction(command, pageContext as PageContext);
  res.json(plan);
});
