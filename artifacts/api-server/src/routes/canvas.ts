...
export function scopedCourseId(userId: string, canvasCourseId: string) {
  return `${userId}__c${canvasCourseId}`;
}

export function scopedAssignmentId(scopedCourse: string, canvasAssignmentId: string) {
...
router.get("/canvas/dashboard", async (req, res): Promise<void> => {
  ...
  try {
    ...
    if (courses.length === 0) {
      res.json({ upcomingCount: 0, overdueCount: 0, completedCount: 0, totalAssignments: 0, avgGrade: null, nextDue: null, courseCount: 0 });
      return;
    }
    ...
    const nextDueAssignment = upcoming.sort(
