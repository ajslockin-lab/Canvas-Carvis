...
import { useGetMe, getGetMeQueryKey, useCanvasSync } from "@workspace/api-client-react";
import { useQueryClient } from "@tanstack/react-query";
import { useToast } from "@/hooks/use-toast";
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { Separator } from "@/components/ui/separator";
import { RefreshCw, Database, Terminal, CheckCircle2, Shield, Settings2 } from "lucide-react";
  ...
  return (
    <div className="max-w-4xl mx-auto animate-in fade-in duration-500">
      <div className="flex items-center gap-3 mb-8">
        ...
        <h1 className="text-2xl font-bold font-mono uppercase tracking-widest text-foreground">System Configuration</h1>
      </div>
      ...
      <div className="space-y-6">
        ...
        <Card className="rounded-none border-border bg-card">
          <CardHeader className="border-b border-border bg-background pb-4">
            ...
            <CardDescription className="font-mono text-xs uppercase">Canvas LMS Connection State</CardDescription>
          </CardHeader>
          <CardContent className="pt-6">
            <div className="flex items-center justify-between p-4 bg-background border border-border">
              <div className="flex items-center gap-4">
                <div className={`w-3 h-3 rounded-full ${me?.canvasConnected ? 'bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.5)]' : 'bg-destructive animate-pulse'}`} />
                <div>
        ...
        <Card className="rounded-none border-border bg-card">
          <CardHeader className="border-b border-border bg-background pb-4">
            ...
            <CardDescription className="font-mono text-xs uppercase">Manual Database Synchronization</CardDescription>
          </CardHeader>
          <CardContent className="pt-6 flex flex-col sm:flex-row gap-6 items-center justify-between">
            <div className="text-sm text-muted-foreground max-w-lg">
              Forces an immediate pull from Canvas LMS endpoints. Updates local cache with latest assignments, courses, and grade calculations. Use if dashboard data appears stale.
            </div>
            <Button 
              onClick={handleSync} 
        ...
        <Card className="rounded-none border-border bg-card opacity-70">
          <CardHeader className="border-b border-border bg-background pb-4">
            ...
            <CardDescription className="font-mono text-xs uppercase">Terminal Interaction Settings</CardDescription>
          </CardHeader>
          <CardContent className="pt-6 space-y-6">
            <div className="flex items-center justify-between">
              <div>
                <h4 className="font-mono text-sm uppercase">Text-to-Speech Output</h4>
                <p className="text-xs text-muted-foreground mt-1">Enable CARVIS vocal responses</p>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                <h4 className="font-mono text-sm uppercase">Continuous Listening</h4>
                <p className="text-xs text-muted-foreground mt-1">Maintain open mic channel (Requires HTTPS)</p>
              </div>
              <Switch disabled />
            </div>
