...
export function createOrb(canvas: HTMLCanvasElement): Orb {
  ...
  function animate() {
    ...
    switch (state) {
      case "idle":
        targetRadius = 28; targetSpeed = 0.2; targetBright = 0.5; targetSize = 0.35; targetLineAmount = 0.15; targetElectronRate = 0; break;
      case "listening":
        targetRadius = 22; targetSpeed = 0.3; targetBright = 0.65; targetSize = 0.4; targetLineAmount = 0.4; targetElectronRate = 0; break;
      case "thinking":
        targetRadius = 16; targetSpeed = 0.5; targetBright = 0.7; targetSize = 0.3; targetLineAmount = 1.0; targetElectronRate = 0.015; break;
      case "speaking":
        targetRadius = 18; targetSpeed = 0.2; targetBright = 0.7; targetSize = 0.4; targetLineAmount = 0.8; targetElectronRate = 0; break;
    }
    ...
    for (let i = 0; i < N; i++) {
      const i3 = i * 3;
      const x = a[i3]; const y = a[i3+1]; const z = a[i3+2];
      const px = phase[i];
    ...
    if (lineAmount > 0.01) {
      ...
      for (let i = 0; i < N && lineCount < MAX_LINES; i += step) {
        const i3 = i * 3;
        const x1 = a[i3]; const y1 = a[i3+1]; const z1 = a[i3+2];
        for (let j = i+step; j < N && lineCount < MAX_LINES; j += step) {
          const j3 = j*3; const dx = a[j3]-x1; const dy = a[j3+1]-y1; const dz = a[j3+2]-z1;
          if (dx*dx+dy*dy+dz*dz < maxDistSq) {
            const idx = lineCount * 6;
            la[idx]=x1; la[idx+1]=y1; la[idx+2]=z1; la[idx+3]=a[j3]; la[idx+4]=a[j3+1]; la[idx+5]=a[j3+2];
            lineCount++;
          }
        }
      }
    ...
    if (state === "thinking") { mat.color.lerp(new THREE.Color(0xff4444), 0.015); lineMat.color.lerp(new THREE.Color(0xa01515), 0.015); }
    else if (state === "speaking") { mat.color.lerp(new THREE.Color(0xff6b3d), 0.015); lineMat.color.lerp(new THREE.Color(0xa01515), 0.015); }
    else { mat.color.lerp(new THREE.Color(0xff1e1e), 0.015); lineMat.color.lerp(new THREE.Color(0x8b0000), 0.015); }
  ...
  return {
    ...
    setAnalyser(a: AnalyserNode | null) { analyser = a; if (a) freqData = new Uint8Array(a.frequencyBinCount); },
    destroy() { destroyed = true; resizeObserver.disconnect(); renderer.dispose(); },
  };
}